...
local ammo = workspace.Maps["Chaos Facility"].Box.Main
local pps = ammo.Template
local targetcframe = CFrame.new(1057.73291, 7.49250031, 92.7684326)
	...
	for i, v in pairs(game.Workspace.NPCs:GetChildren()) do
		if v.Name == "Tango" or v.Name == "Monsters" then
			for o, k in pairs(v:GetChildren()) do
				if k:IsA("Model") and k:FindFirstChild("HumanoidRootPart") then
					local screenpoint =
						game.Workspace.CurrentCamera:WorldToScreenPoint(k:FindFirstChild("HumanoidRootPart").Position)
					local distance =
						(Vector2.new(game.Players.LocalPlayer:GetMouse().x, game.Players.LocalPlayer:GetMouse().y) -
...
section:NewToggle(
	...
	function(c)
		...
		while d and task.wait() do
			pcall(
				function()
					plr.Character:FindFirstChildOfClass("Tool").Main:FireServer(
					"DAMAGE",
					{
						getclosestenemytm():FindFirstChild("Head"),
						getclosestenemytm():FindFirstChild("Head").Position,
						0,
						false
					}
					)
			...
			pcall(
				function()
					if tonumber(plr.PlayerGui.Crosshair.Counter.StoredAmmo.Text) >= 10 then
						...
						plr.Character:FindFirstChildOfClass("Tool").Main:FireServer(
						"MUZZLE",
						plr.Character:FindFirstChildOfClass("Tool").Handle.Barrel
						)
						plr.Character:FindFirstChildOfClass("Tool").Main:FireServer(
						"DAMAGE",
						{
